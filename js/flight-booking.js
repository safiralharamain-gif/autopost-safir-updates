...
let passengersCount = document.getElementById("passengers-count")
let airlineCo       = document.getElementById("airline-co")
let fromAirport     = document.getElementById("from-airport")
let toAirport       = document.getElementById("to-airport")
let flightClass     = document.querySelector(".booking-form .select")
...
sendButton.addEventListener("click", () => {
  if (checked == 'ذهاب فقط') {
    ...
    عدد الأفراد : ${passengersCount.value}%0a
    شركة الطيران : ${airlineCo.value}%0a
    الدرجة : ${flightClass.textContent.trim()}%0a
    من مطار : ${fromAirport.value}%0a
    إلى مطار : ${toAirport.value}%0a
    ${checked}%0a
...
cencelButton.addEventListener("click", () => {
  ...
  airlineCo.value = ''
  flightClass.textContent = 'الإقتصادية'
  fromAirport.value = ''
  goingOnly_Day.value = ''
